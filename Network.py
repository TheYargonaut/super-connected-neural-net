...
import pdb
import Update, Error, Regularize, Activation
...
class SNN_Regressor( object ):
   ...
   def __init__( self, inputSize, outputSize, hiddenSize=100, maxIter=200, independent=True,
                 update=Update.Momentum(), error=Error.Mse(),
                 regularization=Regularize.L2(), hiddenAct=Activation.Tanh() ):
      # structure settings
      ...
      self.dHiddenActivation_ = self.hiddenActivationObj_.df
      self.outputActivation_ = lambda v : v # make settable
      self.dOutputActivation_ = lambda v : 1 # make settable
   ...
   def initialize_( self ):
      ...
      self.oldValueIn_ = np.zeros( self.valueInSize_ )
      self.valueIn_[ 0 ] = 1
      self.weight_ = np.random.normal( scale=np.sqrt( self.valueInSize_ ),
                                       size=( self.valueOutSize_, self.valueInSize_ ) )
      self.dWeight_ = np.zeros( ( self.valueOutSize_, self.valueInSize_ ) )
      self.valueOut_ = np.zeros( self.valueOutSize_ )
      self.dValueOut_ = np.zeros( self.valueOutSize_ )
      self.output_ = np.zeros( self.outputSize_ )
      self.dOutput_ = np.zeros( self.outputSize_ )
   
   def derivative_( self, y ):
      '''compute derivatives given a single expected output'''
      self.dOutput_ = self.dErrorFunc_( y, self.output_ )
      self.dValueOut_[ self.hiddenSize_: ] = self.dOutput_ * self.dOutputActivation_( self.valueOut_[ self.hiddenSize_: ] )
      self.dWeight_[ self.hiddenSize_:, : ] = np.matmul( self.dValueOut_[ self.hiddenSize_: ], 
                                                         self.valueIn_[ None, : ] ) / self.valueInSize_
      # hidden nodes
      for n in range( self.hiddenSize_ - 1, -1, -1 ):
         nOffset = n + self.inputSize_
         dAvg = np.dot( self.dValueOut_, self.weight_[ :, nOffset ] )
         dActivation = self.dHiddenActivation_( self.valueOut_[ n ] )

         self.dValueOut_[ n ] = dAvg * dActivation

         valueSnapshot = np.concatenate( ( self.valueIn_[ :nOffset  ], self.oldValueIn_[ nOffset: ] ) )
         self.dWeight_[ n, : ] = self.dValueOut_[ n ] * valueSnapshot / self.valueInSize_
   
   def fit( self, X, y, batch=0, verbose=True ):
      '''Perform training for maxIter rounds. Returns sequence of round errors.'''
      self.reset()
